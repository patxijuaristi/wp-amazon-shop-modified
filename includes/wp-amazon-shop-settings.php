...
class ACL_Amazon_Product_Settings {
	...
	public function settings_page () {
        ...
        if(!get_option('acl_wpas_chrome_ext_auth')){
            $html.=$this->chrome_addon_notice();
        }
        $html.=$this->premium_themes();
        $tab = '';
			if ( isset( $_GET['tab'] ) && $_GET['tab'] ) {
				$tab .= $_GET['tab'];
			}
    ...
    public function chrome_addon_notice(){
        $html='<div class="notice notice-error is-dismissible">
            <h2 class="wpas-info-page-headline">'.__('WP Amazon Shop : Chrome Extension ', 'wp-amazon-shop').'</h2>
            <p>'.__('Since Amazon does not give access to Product Advertising API (PA API) for beginner & impose a limitation on who has PA API. See more at', 'wp-amazon-shop').'
                <a href="https://docs.aws.amazon.com/AWSECommerceService/latest/DG/TroubleshootingApplications.html#efficiency-guidelines" target="_blank">'.__('Amazon Efficiency Guidelines Docs', 'wp-amazon-shop').'</a> </p>
            <p>'.__('We have introduced WP Amazon Shop Plugin with a charming feature of Chrome Extension that helps to import unlimited Products without AWS & PA API.', 'wp-amazon-shop').'</p>
            <p><strong>'.__('Follow the  ', 'wp-amazon-shop').'</strong><a href="https://amadercode.com/product-import-docs/" target="_blank">'.__('Product Importing Guideline', 'wp-amazon-shop').'</a>'.__(' & Enjoy it!', 'wp-amazon-shop').'</p>
        </div>';
        return $html;
    }
    public function premium_themes(){
	    $html='<div class="acl-product-banner">
        <a target="_blank" href="https://amadercode.com/premium-products/wordpress-themes">
        <img src="https://amadercode.com/free-product/recommended-theme-for-wpas.gif">
        </a>
        </div>';
	    return $html;
    }
