...
class ACL_Amazon_Product_Plugin {
	...
	public function __construct ( $file = '', $version = '1.0.0' ) {
		...
		// Load admin JS & CSS
        if(isset($_GET['page']) && ($_GET['page']=="wp-amazon-shop" ||$_GET['page']=="wp-amazon-shop-basic-import" || $_GET['page']=="wp-amazon-shop-products-import" || $_GET['page']=="wp-amazon-shop-products-import-ext")){
    ...
    //chrome extension
    public function chrome_ext_auth(){
        $auth_token=$_POST['auth_token'];
        if(get_option('acl_wpas_chrome_ext_auth')!=""){
            update_option('acl_wpas_chrome_ext_auth',$auth_token);
        }else{
            add_option('acl_wpas_chrome_ext_auth',$auth_token);
        }

        $message='<div class="notice notice-success is-dismissible">'.
            '<p>'.__( 'Authentication token generated successfully!', 'wp-amazon-shop' ).'</p>'.
            '</div>';
        $redirect_url=strtok(admin_url(), '?').'admin.php?page=wp-amazon-shop-products-import';
        wp_send_json(array('status'=>200,'message'=>$message,'redirect_url'=>$redirect_url));
        wp_die();
    }
